...
#opts_knit$set(root.dir = "/Documents/scratch/dp/dp-public-health/")
...
```{r source_create_allocation_df, echo=FALSE, include = FALSE}
source("scripts/create_allocation_df.r")
```
```{r source_ma_asthma_mcd_data_script, echo=FALSE, include = FALSE}
source("scripts/ma_asthma_mcd_data.r")
```
...
The Census Bureau designed seven and five queries to support the production of person tables in the DDP and v20200527, respectively, and these queries received direct PLB allocations. Query descriptions and PLB allocations are shown in Table 2. The _voting age * Hispanic * race * citizenship_ query, which is used for legislative redistricting, received the largest allocation (50%) in the DDP, and the _total population_ query received the largest allocation (30%) in v20200527. 
...
```{r source_create_sllocation_df, include=FALSE}
#source("scripts/create_allocation_df.r", local = knitr::knit_global())
```


```{r query_allocs_table, echo=FALSE}
...
```{r tda_ddp, echo=FALSE, fig.cap = "Figure 1. Top Down Algorithm used for the v20200527 dataset"}
knitr::include_graphics("/Users/vanriper/Documents/scratch/dp/dp-public-health/figures/TDA_diagrams_journal_articles - Multipass.png")
```
...
The public health data used in our analyses come from three sources. The Centers for Disease Control and Prevention's (CDC) Environmental Public Health Tracking Program (Tracking Program) routinely collects emergency department (ED) visit and hospitalizations data submitted by program recipients and other partners. The 2010 county-level asthma ED visits data were submitted to the Tracking Program by 30 states (DO WE WANT FOOTNOTE TO INCLUDE STATES?) and include ED visits with admission dates from January 1 through December 31, inclusive, for 2010. Asthma ED visits data include all records with asthma (ICD-9-CM code 493) as the primary diagnosis code and include both inpatient and outpatient records as well as transfers to other hospitals. The 2010 county-level acute myocardial infarction (AMI) hospitalizations data were submitted to the Tracking Program by 31 states (SAME AS ABOVE – FOOTNOTE?) and include hospitalizations with admission dates from January 1 through December 31, inclusive, for 2010. AMI hospitalizations data include all records with AMI (ICD-9-CM code 410) as the primary diagnosis code. AMI hospitalizations data excludes transfers, where a patient is discharged from one facility and readmitted to another on the same day. Both the asthma ED visits and AMI hospitalizations data are available by 5-year age groups starting with 0-4 and ending with 85+ years old.

The 2010 asthma ED visits data for Massachusetts towns were downloaded from the Massachusetts Environmental Public Health Tracking database (CITE MEPHT). Town-level asthma ED visits are available for five age groups (0-4, 5-14, 15-34, 35-64, and 65+ years old). 

We use three different sources of 2010 Decennial Census data, each based on a different disclosure control algorithm, when calculating age-adjusted rates. The 2010 Summary File 1 (SF1) data were created using swapping. The 2010 Demonstration Data Product (DDP) data were created using the original Top Down Algorithm, and the v20200527 data were created using the multipass version of the Top Down Algorithm. We used the county subdivision geographic level for the Massachusetts town analyses and the county geographic level for the county-level asthma ED visits and AMI hospitalizations analyses. Age denominators were calculated from table _P12. Sex by Age_.  
...
```{r ma_mcd_perc_diff_sfdenom, echo=FALSE}
...
```{r ma_mcd_adjusted_perc_boxplot, echo=FALSE, warning = FALSE, fig.cap= "Figure X. Boxplots of percent differences between age-adjusted asthma ED visit rates computed from SF1 and DDP (red) and from SF1 and v20200527 (blue) for Massacusetts towns. Towns were assigned to population bins using the invariant total population counts in SF1."}
...
ma_mcd_adjusted_perc %>%
  ...
  geom_boxplot() + 
  scale_x_discrete(name="Total Population (SF1)") +
...
#  ggtitle(paste("Figure X. Percent Difference in Age-Adjusted Asthma ED Visit Rates for MA Towns")) + 
